...
skeleton PSO
{
    ...
    istream& operator>> (istream& is, SetUpParams& setup)
    {
        ...
        while (is.getline(buffer,MAX_BUFFER,'\n'))
        {
            ...
            if (!(strcmp(command,"Binary-pso-params"))) nb_section=1;
            ...
            case 1: // Discrete pso params
                ...
                {
                case 0: setup.delta_min(op*(1.0));break;
                case 1: setup.delta_max(op*1.0);break;
                }
                nb_delta_param++;
                break;
    ...
    {
        for (int i=0;i<_particles.size();i++)
            _particles[i]->initialization();
        //evaluate(); //Innecesario, se hace en StartUp()
    }

    void Swarm::evaluate()
    {
        //cout << "entering Swarm::evaluate()" << endl;
    ...
    /* funcion de proximidad : si numero de genes iguales es mayor o igual que el tama\F1o */
    /* de vecindario*/
    ...
    void Swarm::local_search(Solution& sol)
    {
        ...
        for (int i=0;i<sol.pbm().dimension();i++){
            if (sol1.current(i)==0)
                ...
                sol1.current(i,0);
            if (sol1.current_fitness()<sol2.current_fitness())
                sol2=sol1;
            sol1=sol;
        }
        sol=sol2;
     ...
     * @return int [0,1]
     */
    double Swarm::constrict(double v){
        if (v>10)
            return 1.0;
        if (v<-10)
            return -1.0;
    ...
    /**
     * @brief constrict the value of data betweena
     * @brief we have take into account here the ranges of each variable
     * @return double
     */
    double Swarm::constrict_variable(int pos, double x){
        return x;
    }
    ...
    void Swarm::evolution(int iter){
        double iFactor= 0.0;
        double sFactor= 0.0;
        double delta  = 0.0;
        double lambda = 0.5;
        
        /* here there is the core of the pso algorithm */
        for (unsigned int i=0;i<setup().swarm_size();i++){
            ...
            if (solution(i).current_fitness()<best_solution().current_fitness()){
                best_solution().best_fitness(solution(i).current_fitness());
                for (unsigned int k=0;k<setup().particle_size();k++){
                    best_solution().best(k,solution(i).current(k));
                    best_solution().current(k,solution(i).current(k));
                }
            }/* end if*/

            if (solution(i).current_fitness()!=best_cost()){
                for (unsigned int j=0;j<setup().particle_size();j++){
                    iFactor = setup().individuality_weight()*rand01();    /* individuality weight factor */
                    sFactor = setup().sociality_weight()*rand01();        /* sociality     weight factor */
                    delta = (iFactor*(solution(i).best(j) - solution(i).current(j))
                             +(sFactor*(best_solution().best(j) - solution(i).current(j))));
                    delta=weight(iter,setup().nb_evolution_steps())*solution(i).velocity(j)+delta;
                    solution(i).velocity(j,delta);
                    //x<-x+v
                    if (rand01()<=lambda)
                        solution(i).current(j,ceil(solution(i).current(j)+solution(i).velocity(j)));
                    else
                        solution(i).current(j,floor(solution(i).current(j)+solution(i).velocity(j)));
                }/* end for j*/
            }/*end if */
        }/* end for i*/
        evaluate();
    ...
    void Solver_Seq::DoStep()
    {
        ...
        _swarm.evolution(current_iteration());
    ...
    void Solver_Seq::run ()
    {
        cout << "entering Solver_Seq::run() ..." << endl;
